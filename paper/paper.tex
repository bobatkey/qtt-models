...
In this section, we give an overview of the rules of QTT.

The full rules are presented in \autoref{sec:rules}.

The full rules are presented in the online appendix at
...
\begin{mathpar}
  ...
  \inferrule* [right=Tm-U-Eq]
  {\Gamma \vdash M \stackrel\sigma: \TySet \\
    \Gamma \vdash N_1 \stackrel\sigma: \TyEl(M) \\
    \Gamma \vdash N_2 \stackrel\sigma: \TyEl(M)}
  {\Gamma \vdash N_1 =_M N_2 \stackrel\sigma: \TySet}
\end{mathpar}
Elimination:
\begin{displaymath}
...
\begin{mathpar}
  ...
  {0\Gamma \vdash \TyEl((x \stackrel\rho: M) \to N) \equiv (x \stackrel\rho: \TyEl(M)) \to \TyEl(N)}

  \inferrule* [right=Ty-Eq-El-Bool]
  {0\Gamma~\isctxt}
  {0\Gamma \vdash \TyEl(\BoolTy) \equiv \BoolTy}

  \ldots

  \inferrule* [right=Tm-Eq-Set-Pi-Cong]
  {0\Gamma \vdash M \equiv M' \stackrel0: \TySet \\
    0\Gamma, x \stackrel0: \TyEl(M) \vdash N \equiv N' \stackrel0: \TySet}
  {0\Gamma \vdash (x \stackrel\pi: M) \to N \equiv (x \stackrel\pi: M') \to N' \stackrel0: \TySet}
\end{mathpar}
