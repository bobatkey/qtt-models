...
\begin{abstract}
  ...
  time computation in terms.

  The resulting system smoothly combines reasoning with dependent
  types and restricted computational complexity. \bob{Elaborate after
    Section 3 is written.}We prove the soundness of the systems using
  a realisability technique due to Hofmann and Dal Lago.
\end{abstract}
...
\keywords{type theory, implicit computational complexity, linear logic, computational complexity}
...
monoid structure is addition. Isomorphism of objects $\alpha, \beta$
in such a category amounts to having $M(\alpha, \beta) = 0$. The
conditions in the definition above amount to the usual identity and
composition laws for enriched categories.
...
\paragraph{Agda Formalisation} The natural number resource monoid is
defined in the Agda module \AgdaModule{Algebra.ResourceMonoid.Nat} and the
 polynomial monoids are both defined in
