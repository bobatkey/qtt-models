...
% The goal of this paper is to unify implicit computational complexity
% with full dependent types. ICC enables us to ensure that
% \emph{programs} are within a certain complexity class, while dependent
% types allow us to state and prove that these programs solve the
% problems that we intend them to, and to construct reductions between
% problems. We can marry complexity-controlled implementations with
% problems, paving the way for a \emph{synthetic computational
%   complexity theory}.
...
polytime. SAL has been transformed into a programming language by
\citet{BaillotM04}, and likewise for LLL by
\citet{BaillotGM10}. \citet{hofmann99lfpl} proposed a new programming
...
systems, one based on ideas from SAL and the second more explicitly
based on LFPL. With a view to extending to dependent types in
\autoref{sec:qtt}, we take an approach slightly different to much of
the polytime linear logic literature. We emphasise the use of explicit
datatypes and eliminators, rather than using impredicative encodings
via universal types. We are closer to Hofmann's original LFPL (though
not a later presentation of it by \citet{dallago11realisability}) than
BLL, SAL or LLL.

For this section, the affine linear $\lambda$-calculus we will use
will have linear functions and $\otimes$-products:
\begin{mathpar}
  \inferrule*
  { }
...
substitutes into at most one variable, reducing the size of the term
by one each time.
...
$(x \stackrel1: S) \to T$, where $x$ does not appear in $T$.
...
pattern-matching \cite{atkey18qtt}.
...
standard type theory \cite{atkey18qtt}.
...
QTT rules extend the simply typed rules from
...
(see the encoding of the additive product types in
\cite{atkey18qtt}). Thus, each branch of this tree is polynomial time,
not the whole tree itself.
...
$\cTrue$ exactly when the input is in predicate. Moreover, it is a
...
%   \begin{array}{l}
%     \mathrm{runWithOracle} \stackrel0: \mathrm{ND}\,A \to \ListTy(\BoolTy) \to \mathrm{Maybe}\,A \\
%     \mathrm{runWithOracle}\,(\mathsf{return}\,a)\,\mathit{bs} = \mathsf{just}\,a \\
%     \mathrm{runWithOracle}\,(\mathsf{choice}\,x\,y)\,(\cTrue :: \mathit{bs}) = \mathrm{runWithOracle}\,x\,\mathit{bs} \\
%     \mathrm{runWithOracle}\,(\mathsf{choice}\,x\,y)\,(\cFalse :: \mathit{bs}) = \mathrm{runWithOracle}\,y\,\mathit{bs} \\
%     \mathrm{runWithOracle}\,(\mathsf{choice}\,x\,y)\,\cNil = \mathsf{nothing}
%   \end{array}
% \end{displaymath}
...
change the complexity class. To capture Probabilistic Polynomial time,
we use a (non-iterable) data structure representing trees of
probabilistic choices, where $\mathbb{Q}[0,1]$ is some type of
(non-iterable) rationals in the closed interval $[0,1]$:
\begin{displaymath}
  \begin{array}{l}
    \textbf{data}\,\mathrm{Dist}\,(A : \mathsf{U}) : \mathsf{U}\,\textbf{where} \\
    \quad \mathsf{return} : A \to \mathrm{Dist}\,A \\
    \quad \mathsf{choice} : \mathbb{Q}[0,1] \to (\BoolTy\ to \mathrm{Dist}\,A) \to \mathrm{Dist}\,A
  \end{array}
\end{displaymath}
...
as long as it is correct with probability at least $\frac{1}{3}$:
\begin{displaymath}
  \mathrm{PP}(A,P) =
  \begin{array}[t]{l}
    (f \stackrel1: \mathbf{R}(A \to \mathrm{Dist}(\BoolTy))) \, \otimes
    (a \stackrel1: A) \to (\mathrm{probTrue}\,(\mathbf{R}^{-1}(f)\,a) \geq \frac{1}{3}) \Leftrightarrow P\,a
  \end{array}
\end{displaymath}
Again, problems in PP are easily seen to be closed under polytime
reductions.
...
\citet{atkey18qtt} is a special case of this construction, so we only
sketch a special case here to concentrate on the specifics of
complexity bounded realisability.
...
action of $(\mathbb{N}, \leq)$ on $M$ defined above. The modality
$\oc_n$ has no effect on realising values. It only serves to alter the
resource potentials.
\begin{displaymath}
...
\begin{acks}
  This work was funded by the \grantsponsor{EPSRC}{Engineering and
