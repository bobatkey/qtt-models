...
\acmDOI{10.1145/3632918}
\acmYear{2024}
\copyrightyear{2024}
\acmSubmissionID{popl24main-p453-p}
\acmJournal{PACMPL}
\acmVolume{8}
\acmNumber{POPL}
\acmArticle{76}
...
\paragraph{Contributions and Content}
...
$\lambda$-terms always takes a number of steps linearly proportional
...
booleans, which are non-recursive and so would not allow
iteration anyway:
\begin{mathpar}
  \inferrule*
  { }
...
%\autoref{sec:rules}.
the exteneded version of this paper \cite{atkey2023polydep-online}.
...
\subsection{Non-Iterable Data Types}
...
but in order to ensure the polytime complexity guarantees we only permit
matching without recursion in the $\sigma = 1$ fragment. Here is the rule for
...
rule, which is the standard dependent eliminator for lists except that
...
iterable datatype. In both our \ConsFree{} and LFPL-style QTT systems,
we use a natural number datatype.

The \ConsFree{} system cannot allow the programmer to construct natural
...
definitionally that $\mathrm{dupNat}$ acts as its name implies:
\begin{mathpar}
  \inferrule*
  {\Gamma \vdash M \stackrel0: \tyNat}
...
Unlike the natural number iterator in the \ConsFree{} system, this
...
annotated $0$, which allows us to use $M_n$ twice even when we are in
the $\sigma = 1$ fragment.
...
explicit constructions are essential for constructing models of our systems.
...
though this could be generalised. We use $\eta[i]$ to access the $i$th
variable in the environment, counting from the right. The evaluation
rules are mostly as one would expect, except that the application rule
...
well-scoped syntax, which ensures that all variable accesses are well
defined.
...
$\PlusPoly$ resource monoids will be used for the \ConsFree{} and
