...
will have linear functions and $\otimes$-products:\bob{CHECK THAT THIS CHANGE TO THE VAR RULE IS OK, MENTION THAT EXCHANGE IS IMPLICIT}
\begin{mathpar}
  \inferrule*
  { }
  {\Gamma, x : A \vdash x : A}
...
\bob{Mention equality rules, and put them in an appendix; also
  Conversion rule is missing!}
...
\bob{$\beta\eta$-laws for $\Pi$-types}
...
\bob{$\beta\eta$-laws for $\Sigma$-types, with wrinkle in the $\sigma = 1$ fragment}
...
\bob{$\beta$ laws in the $\sigma = 0$ fragment}
...
\bob{$\beta$ laws in both fragments}
...
\bob{Equality rules (see response)}
...
\bob{Check PP vs BPP. Also cite Dal Lago PP-time paper}
...
complexity bounded realisability. \bob{state clearly that Atkey 2018
  has a mistake, and this construction here is more general}
...
resource potentials.\bob{fix this to match the Agda implementation: special case $0$}
\begin{displaymath}
