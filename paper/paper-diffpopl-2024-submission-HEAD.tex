...
%DIF DEL paper/paper-oldtmp-238458.tex   Thu Oct 26 09:52:19 2023
%DIF ADD paper/paper-newtmp-238458.tex   Thu Oct 26 09:52:19 2023
%DIF < \documentclass[acmsmall,review,screen,anonymous]{acmart}
%DIF -------
...
\begin{abstract}
  ...
  method. Both of these are \DIFdelbegin \DIFdel{extend }\DIFdelend \DIFaddbegin \DIFadd{extended }\DIFaddend to full dependent types via
...
<ccs2012>
   <concept>
       <concept_id>\DIFdelbegin \DIFdel{10010520.10010553.10010562}\DIFdelend \DIFaddbegin \DIFadd{10003752.10003790.10003801}\DIFaddend </concept_id>
       <concept_desc>\DIFdelbegin \DIFdel{Computer systems organization~Embedded systems}\DIFdelend \DIFaddbegin \DIFadd{Theory of computation~Linear logic}\DIFaddend </concept_desc>
       <concept_significance>500</concept_significance>
       </concept>
   <concept>
       <concept_id>\DIFdelbegin \DIFdel{10010520.10010575.10010755}\DIFdelend \DIFaddbegin \DIFadd{10003752.10003790.10011740}\DIFaddend </concept_id>
       <concept_desc>\DIFdelbegin \DIFdel{Computer systems organization~Redundancy}\DIFdelend \DIFaddbegin \DIFadd{Theory of computation~Type theory}\DIFaddend </concept_desc>
       <concept_significance>\DIFdelbegin \DIFdel{300}\DIFdelend \DIFaddbegin \DIFadd{500}\DIFaddend </concept_significance>
       </concept>
   <concept>
       <concept_id>\DIFdelbegin \DIFdel{10010520.10010553.10010554}\DIFdelend \DIFaddbegin \DIFadd{10003752.10003777.10003778}\DIFaddend </concept_id>
       <concept_desc>\DIFdelbegin \DIFdel{Computer systems organization~Robotics}\DIFdelend \DIFaddbegin \DIFadd{Theory of computation~Complexity classes}\DIFaddend </concept_desc>
       <concept_significance>\DIFdelbegin \DIFdel{100}\DIFdelend \DIFaddbegin \DIFadd{500}\DIFaddend </concept_significance>
       </concept>
   <concept>
       <concept_id>\DIFdelbegin \DIFdel{10003033.10003083.10003095}\DIFdelend \DIFaddbegin \DIFadd{10003752.10003777.10003787}\DIFaddend </concept_id>
       <concept_desc>\DIFdelbegin \DIFdel{Networks~Network reliability}\DIFdelend \DIFaddbegin \DIFadd{Theory of computation~Complexity theory and logic}\DIFaddend </concept_desc>
       <concept_significance>\DIFaddbegin \DIFadd{300</concept_significance>
       </concept>
   <concept>
       <concept_id>10003752.10010124.10010131.10010137</concept_id>
       <concept_desc>Theory of computation~Categorical semantics</concept_desc>
       <concept_significance>}\DIFaddend 100</concept_significance>
       </concept>
 </ccs2012>
\end{CCSXML}

\DIFdelbegin %DIFDELCMD < \ccsdesc[500]{Computer systems organization~Embedded systems}
%DIFDELCMD < \ccsdesc[300]{Computer systems organization~Redundancy}
%DIFDELCMD < \ccsdesc{Computer systems organization~Robotics}
%DIFDELCMD < \ccsdesc[100]{Networks~Network reliability}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \ccsdesc[500]{Theory of computation~Linear logic}
\ccsdesc[500]{Theory of computation~Type theory}
\ccsdesc[500]{Theory of computation~Complexity classes}
\ccsdesc[300]{Theory of computation~Complexity theory and logic}
...
Type \DIFdelbegin \DIFdel{theory }\DIFdelend \DIFaddbegin \DIFadd{Theory }\DIFaddend is often claimed to be ideal for Computer Science,
...
verify. From a broader Computer Science view, however, \DIFdelbegin \DIFdel{type theory
}\DIFdelend \DIFaddbegin \DIFadd{Type Theory
}\DIFaddend lacks the ability to talk about the very thing that makes Computer
...
\DIFdelbegin \DIFdel{Typical }\DIFdelend \DIFaddbegin \DIFadd{Typically, }\DIFaddend Type Theory only speaks of the public face that programs
...
programming language in \DIFdelbegin \DIFdel{type theory}\DIFdelend \DIFaddbegin \DIFadd{Type Theory}\DIFaddend , for example \cite{GueneauCP18},
...
in \DIFdelbegin \DIFdel{type theory}\DIFdelend \DIFaddbegin \DIFadd{Type Theory}\DIFaddend . These techniques deliver only \emph{conspicuous
  consumption}, not speaking of the real resources consumed.

In this paper, we propose a method for extending dependent \DIFdelbegin \DIFdel{type theory
}\DIFdelend \DIFaddbegin \DIFadd{Type Theory
}\DIFaddend with a means for constraining the intrinsic computational complexity
...
non-deterministic and \DIFaddbegin \DIFadd{bounded-error }\DIFaddend probabilistic polynomial time.
...
programs' resource usage \DIFaddbegin \DIFadd{as well as their extensional behaviour}\DIFaddend , and
...
%DIF <  The goal of this paper is to unify implicit computational complexity
%DIF <  with full dependent types. ICC enables us to ensure that
%DIF <  \emph{programs} are within a certain complexity class, while dependent
%DIF <  types allow us to state and prove that these programs solve the
%DIF <  problems that we intend them to, and to construct reductions between
%DIF <  problems. We can marry complexity-controlled implementations with
%DIF <  problems, paving the way for a \emph{synthetic computational
%DIF <    complexity theory}.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \item Section 5: Extensions
%DIF <    \begin{enumerate}
%DIF <    \item Explicit Complexity via Diamond Counting
%DIF <    \item Poly-time univalence
%DIF <    \item Towards P- and NP-completeness proofs. Issue here is the
%DIF <      extensionality. Towards a synthetic (poly-time based)
%DIF <      computational complexity theory.
%DIF <    \end{enumerate}
%DIF <  \item Section 6: Related Work
%DIF <    \begin{enumerate}
%DIF <    \item ICC stuff
%DIF <      \begin{enumerate}
%DIF <      \item LFPL
%DIF <      \item CONS-free
%DIF <      \item Soft Affine Logic; which has the same underlying idea as
%DIF <        CONS-free (is this a novel observation?)
%DIF <      \item Other systems based on information flow.
%DIF <      \end{enumerate}
%DIF <    \item Explicit complexity: lambda-amor, and things by Deepak Garg
%DIF <      and Marco Gaboardi.
%DIF <    \item Recent CMU stuff on calf
%DIF <    \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Contributions and Content}
...
\item We formulate two systems that combine linear type theory for
  ...
  Type Theory. The \DIFdelbegin \DIFdel{system is }\DIFdelend \DIFaddbegin \DIFadd{systems are }\DIFaddend presented in \autoref{sec:qtt}. The
  ...
  types.
%DIF <  We explain in \autoref{Sec:realisability-future} how this
  %DIF <  will have applications in QTT beyond the polytime systems presented
  %DIF <  here.
\item We demonstrate the utility of the combination of polytime and
  ...
  Non-deterministic Polynomial (NP) time and \DIFaddbegin \DIFadd{Bounded-error
  }\DIFaddend Probabilistic Polynomial (\DIFdelbegin \DIFdel{PP}\DIFdelend \DIFaddbegin \DIFadd{BPP}\DIFaddend ) time. Since these classes rely on
...
is usually proven to be complete for the relevant complexity class \DIFdelbegin \DIFdel{, }\DIFdelend by
...
Viewing logical systems though the \DIFdelbegin \DIFdel{lens of the }\DIFdelend Curry-Howard correspondence, the
...
iteration.
\DIFdelbegin \DIFdel{In this section, we
review the use of
linear types to capture polytime by presenting two
systems, one based on ideas from SAL and
the second more explicitly
based on LFPL. }\DIFdelend \DIFaddbegin 

\DIFadd{There are at least two ways that a functional programming language can
...
\DIFaddend With a view to extending to dependent types in \autoref{sec:qtt}, we
take an approach slightly different to much of the polytime linear
logic literature. We \DIFdelbegin \DIFdel{emphasise the use of }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend explicit datatypes and eliminators, rather
...
\DIFaddbegin \DIFadd{In this section, we review the use of linear types to capture polytime
by presenting two systems, one based on ideas from SAL and the second
more explicitly based on LFPL.
}

\subsection{\DIFadd{Affine Linear $\lambda$-Calculus}}

\DIFaddend For this section, the affine linear $\lambda$-calculus we will use
will have linear functions and $\otimes$-products\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{. Contexts are
treated up to permutation of entries, so uses of exchange are
implicit.
}\DIFaddend \begin{mathpar}
  \inferrule*
  { }
  {\Gamma, x : A \DIFdelbegin \DIFdel{, \Gamma' }\DIFdelend \vdash x : A}
...
substitutes \DIFaddbegin \DIFadd{each term }\DIFaddend into at most one variable, reducing the size of
...
\emph{Cons-Free} system.  \DIFaddbegin \DIFadd{Because we cannot construct values of type
$\tyNat$ within the system, complete programs in this system are open
terms as we explained at the start of this section.
}

\DIFaddend The second prohibited feature is the ability to duplicate values of
...
%DIFDELCMD <     I_{k+1} : (\tyNat \otimes \mathit{St}) \multimap (\tyNat \otimes \mathit{St}) \\
%DIFDELCMD <     I_{k+1} = \lambda (n,s).\,\tmRec\,n\,\{
%DIFDELCMD <     \begin{array}[t]{@{}lcl}
%DIFDELCMD <       \conZero(d)&\mapsto&(\conZero(d),s); \\
%DIFDELCMD <       \conSucc(d,(n,s)) &\mapsto& \Let\,(n,s)=I_k\,(n,s)\,\In\,(\conSucc(d,n), f\,s) \}
%DIFDELCMD <     \end{array}%%%
...
the degree of the nested iterator directly. Rather\DIFaddbegin \DIFadd{, }\DIFaddend the iterator $I_k$
\DIFaddbegin \DIFadd{on the input $n$ }\DIFaddend performs $\binom{n}{k}$ iterations. As observed by
...
%DIF <  \subsection{Explicit Resource Tracking}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  \begin{enumerate}
%DIF <  \item We still require diamonds to build zero and successor, but the
%DIF <    iterator does not give them back.
%DIF <  \item In the simply typed case, we can annotate types with the static
%DIF <    amount of potential they have per constructor.
%DIF <  \end{enumerate}
%DIFDELCMD < 
...
syntactically \DIFdelbegin \DIFdel{yield }\DIFdelend \DIFaddbegin \DIFadd{yielding }\DIFaddend multiple uses of the same variable.
...
area of systems that use semiring \DIFdelbegin \DIFdel{annotatons }\DIFdelend \DIFaddbegin \DIFadd{annotations }\DIFaddend to measure resource usage
...
\DIFaddbegin \DIFadd{In this section, we give an overview of the rules of QTT. The full
rules, including all equality rules, are presented in the online
appendix for this paper \mbox{%DIFAUXCMD
\cite{atkey2023polydep-online}}\hskip0pt%DIFAUXCMD
.
}

%DIF > The full rules are presented in \autoref{sec:rules}.

\DIFaddend \subsubsection{Natural-number usages}
...
logic, since \DIFdelbegin \DIFdel{$m \leq 0$ }\DIFdelend \DIFaddbegin \DIFadd{$m \sqsubseteq 0$}\footnote{\DIFadd{Reverse ordering!}} \DIFaddend for all $m$,
...
\begin{mathpar}
  ...
  \inferrule*
  {\Gamma \vdash M \DIFaddbegin \stackrel{\DIFadd{\sigma}}\DIFaddend : S \\ \Gamma' \sqsubseteq \Gamma}
  {\Gamma' \vdash M \DIFaddbegin \stackrel{\DIFadd{\sigma}}\DIFaddend : S}
\end{mathpar}
...
\DIFaddbegin \DIFadd{As usual, definitional equality of types impacts typing of terms via
the conversion rule:
}\begin{displaymath}
  \inferrule*
  {\DIFadd{\Gamma \vdash M : S }\\ \DIFadd{0\Gamma \vdash S \equiv T~\istype}}
  {\DIFadd{\Gamma \vdash M : T}}
\DIFadd{}\end{displaymath}
\DIFadd{Like type formation, definitional equality of types always takes place
...
\DIFaddend \subsubsection{$\Pi$- and $\Sigma$-types}
...
$T$. \DIFaddbegin \DIFadd{$\Pi$-types support the usual $\beta\eta$ definitional
...
fragment \DIFdelbegin \DIFdel{, }\DIFdelend where we do not need to care about polytime
realisability. Formation and introduction are given by the rules:
\begin{mathpar}
  \inferrule*
  {0\Gamma \vdash S\DIFaddbegin \DIFadd{~\istype }\DIFaddend \\ 0\Gamma, x \stackrel0: S \vdash T\DIFaddbegin \DIFadd{~\istype}\DIFaddend }
  {0\Gamma \vdash (x \stackrel\pi: S) \otimes T\DIFaddbegin \DIFadd{~\istype}\DIFaddend }
...
\end{mathpar}
As with $\Pi$\DIFdelbegin \DIFdel{-typed}\DIFdelend \DIFaddbegin \DIFadd{-types}\DIFaddend , the first component of a $\Sigma$\DIFdelbegin \DIFdel{type }\DIFdelend \DIFaddbegin \DIFadd{-type }\DIFaddend is
...
pattern-matching \cite{atkey18qtt}. \DIFaddbegin \DIFadd{$\Sigma$- and $I$ types support
...
commuting conversions \mbox{%DIFAUXCMD
\cite{Barber1996} }\hskip0pt%DIFAUXCMD
for $\mathrm{let}$ in the
...
standard type theory \cite{atkey18qtt}. \DIFaddbegin \DIFadd{For our examples below, we do
...
\cite{atkey18qtt}\DIFdelbegin \DIFdel{, but we repeat it here for completeness}\DIFdelend . Booleans offer no possibility for iteration, but it
...
resources between the branches is expected. \DIFaddbegin \DIFadd{Booleans and their
...
dependently typed case analysis on lists\DIFaddbegin \DIFadd{, which also obeys the usual
...
$\sigma = 0$ \DIFaddbegin \DIFadd{fragment }\DIFaddend recursor for lists has the following typing
...
\end{mathpar}
\DIFaddbegin \DIFadd{This eliminator also obeys the usual $\beta$-equality laws for a list
...
depth\DIFdelbegin \DIFdel{determined statically}\DIFdelend \DIFaddbegin \DIFadd{, determined statically by the program}\DIFaddend . To write programs that do
...
\begin{mathpar}
  \inferrule*
  {\Gamma\DIFdelbegin \DIFdel{\vdash}\DIFdelend \DIFaddbegin \DIFadd{~\isctxt}\DIFaddend }
...
\end{mathpar}
Anyone who has reasoned about the metatheory of\DIFaddbegin \DIFadd{, }\DIFaddend or implemented a type
checker for\DIFaddbegin \DIFadd{, }\DIFaddend dependent types will view this rule with unease as it
...
form. Disregarding the usage annotations, \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend has the same type
structure as the normal dependently typed recursor for \DIFdelbegin \DIFdel{natural
numbers}\DIFdelend \DIFaddbegin \DIFadd{naturals}\DIFaddend :
\begin{displaymath}
  \mprset{flushleft}
  \inferrule*
...
\DIFaddbegin \DIFadd{This eliminator cannot have a $\beta$-equality in the $\sigma = 1$
...
\DIFaddend The reader is invited to compare this dependently typed rule with the
...
unit type. This allows us to \DIFdelbegin \DIFdel{use }\DIFdelend freely use diamonds in types, and to
...
\DIFadd{Note that the fact that the variable $n$ in the $N_s$ term is
annotated $0$, which allows us to duplicate it even when we are in the
$\sigma = 1$ fragment.
}

\DIFaddend Just as for the Cons-free system iterator above, in the $\sigma = 0$
...
\DIFaddbegin \DIFadd{The equality rules for $\Rtype$ state that the two operations are
...
\DIFaddend With just the rules given here, the type $\Rtype(A)$ is no more than a
...
}\DIFdelend construction. However, sticking with just the natural numbers, we can
...
arbitrarily duplicable. Lifting this \DIFdelbegin \DIFdel{restrict }\DIFdelend \DIFaddbegin \DIFadd{restriction }\DIFaddend by means of some
...
account for the \DIFaddbegin \DIFadd{comparisons between elements.
}\DIFaddend 
...
decision problem $(A,P)$ is \DIFdelbegin \DIFdel{deciable }\DIFdelend \DIFaddbegin \DIFadd{decidable }\DIFaddend in polytime. We make \DIFdelbegin \DIFdel{two }\DIFdelend \DIFaddbegin \DIFadd{three }\DIFaddend notes
about this definition: (i) \DIFaddbegin \emph{\DIFadd{proofs}} \DIFadd{of $\mathrm{PTIME}(A,P)$, are
carried out in the $\sigma = 0$ fragment, where we have the full power
of Type Theory to aid us; (ii) }\DIFaddend this definition is intrinsic, in the
...
counting; and (\DIFdelbegin \DIFdel{ii}\DIFdelend \DIFaddbegin \DIFadd{iii}\DIFaddend ) we have defined problems to have arbitrary types
...
(see the encoding of the additive product types \DIFdelbegin \DIFdel{in
\mbox{%DIFAUXCMD
\cite{atkey18qtt}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{by
\mbox{%DIFAUXCMD
\citet{atkey18qtt}}\hskip0pt%DIFAUXCMD
}\DIFaddend ). Thus, each branch of this tree is polynomial
...
\begin{displaymath}
  \mathrm{NP}(A,P) =
  \DIFdelbegin %DIFDELCMD < \begin{array}[t]{@{}l}
%DIFDELCMD <     (f \stackrel1: \mathbf{R}(A \to \mathrm{ND}(\BoolTy)))\, \otimes \\
%DIFDELCMD <     \quad (a \stackrel1: A) \to \left((\mathit{bs} \stackrel1: \ListTy(\BoolTy)) \otimes (\mathrm{runWithOracle}\,(\mathbf{R}^{-1}(f)\,a)\,\mathit{bs} = \mathsf{just}\,\cTrue)\right) \Leftrightarrow P\,a
%DIFDELCMD <   \end{array}%%%
\DIFdelend \DIFaddbegin \begin{array}[t]{@{}l}
...
$\cTrue$ exactly when the input \DIFdelbegin \DIFdel{is in }\DIFdelend \DIFaddbegin \DIFadd{satisfies the }\DIFaddend predicate. Moreover, it is a
quick matter of programming to see that problems in NP are closed
under the type of polytime reductions given above.

%DIF <  \begin{displaymath}
%DIF <    \begin{array}{l}
%DIF <      \mathrm{runWithOracle} \stackrel0: \mathrm{ND}\,A \to \ListTy(\BoolTy) \to \mathrm{Maybe}\,A \\
%DIF <      \mathrm{runWithOracle}\,(\mathsf{return}\,a)\,\mathit{bs} = \mathsf{just}\,a \\
%DIF <      \mathrm{runWithOracle}\,(\mathsf{choice}\,x\,y)\,(\cTrue :: \mathit{bs}) = \mathrm{runWithOracle}\,x\,\mathit{bs} \\
%DIF <      \mathrm{runWithOracle}\,(\mathsf{choice}\,x\,y)\,(\cFalse :: \mathit{bs}) = \mathrm{runWithOracle}\,y\,\mathit{bs} \\
%DIF <      \mathrm{runWithOracle}\,(\mathsf{choice}\,x\,y)\,\cNil = \mathsf{nothing}
%DIF <    \end{array}
%DIF <  \end{displaymath}
\DIFaddbegin \subsubsection{\DIFadd{Bounded-error Probabilistic Polynomial Time}}
\DIFaddend 

\DIFdelbegin \subsubsection{\DIFdel{Probabilistic Polynomial Time}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend By changing the computation effects supplied to a program, we can
change the complexity class. To capture \DIFaddbegin \DIFadd{the class BPP of Bounded-error
}\DIFaddend Probabilistic Polynomial time \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{AroraBarak09}}\hskip0pt%DIFAUXCMD
}\DIFaddend , we use a
...
\begin{displaymath}
  \DIFdelbegin %DIFDELCMD < \begin{array}{l}
%DIFDELCMD <     \textbf{data}\,\mathrm{Dist}\,(A : \mathsf{U}) : \mathsf{U}\,\textbf{where} \\
%DIFDELCMD <     \quad \mathsf{return} : A \to \mathrm{Dist}\,A \\
%DIFDELCMD <     \quad \mathsf{choice} : \mathbb{Q}[0,1] \to (\BoolTy\ to \mathrm{Dist}\,A) \to \mathrm{Dist}\,A
%DIFDELCMD <   \end{array}%%%
\DIFdelend \DIFaddbegin \begin{array}{l}
...
as long as it is correct with probability at least \DIFdelbegin \DIFdel{$\frac{1}{3}$:
}\DIFdelend \DIFaddbegin \DIFadd{$\frac{2}{3}$:
}\DIFaddend \begin{displaymath}
  \DIFdelbegin \DIFdel{\mathrm{PP}}\DIFdelend \DIFaddbegin \DIFadd{\mathrm{BPP}}\DIFaddend (A,P) =
  \DIFdelbegin %DIFDELCMD < \begin{array}[t]{l}
%DIFDELCMD <     (f \stackrel1: \mathbf{R}(A \to \mathrm{Dist}(\BoolTy))) \, \otimes
%DIFDELCMD <     (a \stackrel1: A) \to (\mathrm{probTrue}\,(\mathbf{R}^{-1}(f)\,a) \geq \frac{1}{3}) \Leftrightarrow P\,a
%DIFDELCMD <   \end{array}%%%
\DIFdelend \DIFaddbegin \begin{array}[t]{l}
  ...
  \end{array}\DIFaddend 
\end{displaymath}
Again, problems in \DIFdelbegin \DIFdel{PP }\DIFdelend \DIFaddbegin \DIFadd{BPP }\DIFaddend are easily seen to be closed under polytime
reductions.

\DIFaddbegin \DIFadd{Probabilistic Polynomial time has previously been considered in the
setting of implicit computational complexity by
\mbox{%DIFAUXCMD
\citet{dallago_et_al:LIPIcs.MFCS.2021.35} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{LagoT15}}\hskip0pt%DIFAUXCMD
. In both
...
semantic nature \mbox{%DIFAUXCMD
\cite{AroraBarak09}}\hskip0pt%DIFAUXCMD
...
Agda module \AgdaModule{MachineModel}. We use an \DIFdelbegin \DIFdel{intrinsicaly
}\DIFdelend \DIFaddbegin \DIFadd{intrinsically
}\DIFaddend well-scoped syntax, which ensures that all variable look up operations
...
only supply as much fuel as is contained in the potential.  %DIF <  This makes
%DIF <  it suitable for amortised analysis, where we directly store potential
%DIF <  in data structures.
For the
...
%DIF <  FIXME: difference between this definition and Hofmann-Dal Lago's: we
%DIF <  use polynomials with natural number coefficients instead of a function
%DIF <  that is bounded by a polynomial.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsubsection{Resource sub-monoids}
...
(QCwFs). \DIFdelbegin \DIFdel{This class of
...
preorders. The BCI-realisability model construction given by
\mbox{%DIFAUXCMD
\citet{atkey18qtt} }\hskip0pt%DIFAUXCMD
is a special case of this construction, so we only
sketch a special case here to concentrate on the
specifics of complexity bounded realisability }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{atkey18qtt} }\hskip0pt%DIFAUXCMD
constructs QCwFs from certain Linear
...
category of }\emph{\DIFadd{assemblies}} \DIFadd{over the LCA, where it ought to be the
category of sets paired with realisability relations, with no
guarantee that all elements be realisable}\DIFaddend .

\DIFaddbegin \DIFadd{Here, we fix the mistake of \mbox{%DIFAUXCMD
\citet{atkey18qtt}}\hskip0pt%DIFAUXCMD
, and provide a more
...
\DIFaddend \begin{definition}\label{def:indexed-linear-preorder}
  A \emph{$\mathbb{N}$-linear preorder}\DIFaddbegin \footnote{\DIFadd{We specialise to the semiring $\mathbb{N}$ here, but the same definition works for any suitable semiring $\mathcal{R}$.}} \DIFaddend is a preordered set
  $(L, \leq)$\DIFdelbegin \DIFdel{with
...
\item Define scaling of objects of $\cat{L}$ by
  ...
  as $(A, X \otimes Y)$.
\item For each set $A$, define the collection of \DIFdelbegin \DIFdel{semantics }\DIFdelend \DIFaddbegin \DIFadd{semantic }\DIFaddend types
...
\item For each $A$ and $(B,X) \in \Ty(A)$, the $\sigma = 0$ fragment
  ...
  context interpretation $(A, X)$ in $\cat{L}$ and type \DIFdelbegin \DIFdel{intepretation
  }\DIFdelend \DIFaddbegin \DIFadd{interpretation
  }\DIFaddend $(B,Y) \in \Ty(A)$, the $\sigma = 1$ fragment terms
...
\item Realisability reflection for a type $(B,X) \in \Ty(A)$ is
  \DIFdelbegin \DIFdel{intepreted }\DIFdelend \DIFaddbegin \DIFadd{interpreted }\DIFaddend as the type
...
$X \leq Y$ to hold iff there exists \DIFaddbegin \DIFadd{a }\DIFaddend realising expression
...
action of $(\mathbb{N}, \leq)$ on $M$ defined above. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{When $n > 0$, the
}\DIFaddend modality $\oc_n$ has no effect on realising values. It only serves to
alter the resource potentials. \DIFaddbegin \DIFadd{In the $n = 0$, case the realising
value must be $*$, in order to satisfy the $\oc_0 X \cong I$ condition
in }\defref{def:indexed-linear-preorder} \DIFadd{3(a):
}\DIFaddend \begin{displaymath}
  \DIFdelbegin %DIFDELCMD < \oc%%%
\DIFdel{_n\,X = \{(a,\alpha,v) \mid \exists \alpha'.\,M(n \cdot \alpha', \alpha) = 0 }%DIFDELCMD < \land %%%
\DIFdel{(a, \alpha', v) \in X \}
}\DIFdelend \DIFaddbegin \begin{array}{lcl}
...
and $\mathit{ff}$ is represented by $\synFalse$. In both \DIFdelbegin \DIFdel{case}\DIFdelend \DIFaddbegin \DIFadd{cases}\DIFaddend , we
...
$X \in L(\Sigma a:A.\, B a)$, then \DIFdelbegin \DIFdel{we }\DIFdelend the resourced lists predicate
...
%DIF <  \subsubsection{Realising QTT}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Summing up the previous sections, we have the following key theorem
%DIF <  paving the way to polytime soundness:
%DIF <  \begin{theorem}\label{thm:basic-qtt-realisability-model}
%DIF <    For every resource monoid with a distinguished sub-monoid
%DIF <    $(M, M_0)$, we obtain a realisability model of $\mathbb{N}$-QTT with
%DIF <    $\Pi$ and $\Sigma$ types, a universe, extensional equality types,
%DIF <    realisability reflection, and non-iterable datatypes.
%DIF <  \end{theorem}
%DIFDELCMD < 
...
potential for whole \DIFdelbegin \DIFdel{programs2}\DIFdelend \DIFaddbegin \DIFadd{programs}\DIFaddend , meaning that it does not make any
...
\begin{theorem}[Soundness for the Cons-free System]
  ...
  \DIFdelbegin \DIFdel{$\llbracket M \rrbracket(n) \in \llbracket X \rrbracket(n)$}\DIFdelend \DIFaddbegin \DIFadd{$\llbracket M \rrbracket(n) \in \llbracket T \rrbracket(n)$}\DIFaddend .
\end{theorem}
...
ideas, via the work of \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{HofmannJ03}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{HofmannJ03}}\hskip0pt%DIFAUXCMD
}\DIFaddend . More details are to be
found in the survey paper of \citet{HoffmannJ22}. \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{Another }\DIFaddend approach is
...
in Coq. All of these \DIFaddbegin \DIFadd{tick-counting }\DIFaddend techniques rely on the programmer
...
\cite{CervesatoP02,KrishnaswamiPB15,Vakar14} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{CervesatoP02}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{KrishnaswamiPB15}}\hskip0pt%DIFAUXCMD
, and \mbox{%DIFAUXCMD
\citet{Vakar14}
}\hskip0pt%DIFAUXCMD
}\DIFaddend which all use a strict separation between linear and non-linear
...
of \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{MoonEO21,ChoudhuryEEW21,Abel18}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{MoonEO21}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{ChoudhuryEEW21}}\hskip0pt%DIFAUXCMD
, and \mbox{%DIFAUXCMD
\citet{Abel18}}\hskip0pt%DIFAUXCMD
}\DIFaddend . These
...
not include a universe type\DIFaddbegin \DIFadd{, which we used in }\autoref{sec:classes} \DIFadd{to
be able to characterise complexity classes as predicates decidable in
restricted complexity}\DIFaddend .
...
\DIFdelbegin \DIFdel{implemenation }\DIFdelend \DIFaddbegin \DIFadd{implementation }\DIFaddend of QTT, but cannot be used directly because its facility
...
\DIFdelbegin \DIFdel{It seems }\DIFdelend \DIFaddbegin \DIFadd{We have been able to characterise the classes NP and BPP in terms of
our underlying characterisation of P (}\autoref{sec:classes}\DIFadd{). It seems
straighforward to extend this to related classes like coNP, RP,
etc. It also seems }\DIFaddend feasible to adapt the techniques presented here to
other complexity classes such as LOGSPACE and ELEMENTARY, given the
simply typed linear systems mentioned above. \DIFdelbegin \DIFdel{Also, our }\DIFdelend \DIFaddbegin \DIFadd{Complexity classes based
on circuits may be more challenging, but we do now have a way to
characterise circuits that are generatable in polynomial
time.
}

\DIFadd{Our }\DIFaddend construction already includes soundness of a system with intrinsic
...
yield a system that tracks \DIFdelbegin \DIFdel{costs intrinsically and
precisely
}\DIFdelend \DIFaddbegin \DIFadd{the intrinsic cost of programs precisely
and explicitly}\DIFaddend .
...
characterise the class NP, as we did in \autoref{sec:np-class}, \DIFaddbegin \DIFadd{and it
is a ``matter of programming'' to show that 3-SAT is in NP, }\DIFaddend we cannot
...
writing polytime \DIFdelbegin \DIFdel{enodings }\DIFdelend \DIFaddbegin \DIFadd{encodings }\DIFaddend into 3-SAT. We hope that the addition of
...
\begin{acks}
  ...
  was funded by the \grantsponsor{EPSRC}{Engineering and Physical
    Sciences Research Council}{https://www.ukri.org/about-us/epsrc/}:
  Grant number \grantnum{EPSRC}{EP/T026960/1}, \emph{AISEC: AI Secure
