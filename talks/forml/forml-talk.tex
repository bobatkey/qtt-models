...
\begin{frame}
  ...
  \youtem Proper datatypes (definitely no Church encodings)
...
\begin{frame}
  ...
  % From “Higher type recursion, rami cation and polynomial time” by
  % Bellantoni, Niggl and Schwictenberg. Annals of Pure and Applied
  % Logic 104 (2000) 17–30
...
\begin{frame}
  ...
  \begin{mathpar}
    \inferrule*
    {\Gamma \vdash}
    {\Gamma \vdash \mathsf{zero} \stackrel\sigma: \TyNat}

    \inferrule*
    {\Gamma \vdash N \stackrel\sigma: \TyNat}
    {\Gamma \vdash \mathsf{suc}(N) \stackrel\sigma: \TyNat}
\end{mathpar}
  ...
  \begin{mathpar}
    ...
    \inferrule*
    {\Gamma \vdash n \stackrel0: \TyNat}
    {\Gamma \vdash * \stackrel0: \Diamond^{(p_1,\dots,p_k)(n)}}
  \end{mathpar}
  ...
  \begin{mathpar}
    \begin{array}{l@{\hspace{0.3em}}c@{\hspace{0.3em}}l}
      ...
      \mathrm{shift} &:& (n \stackrel0: \TyNat) \to \Diamond^{(p_1,\dots,p_k)}(\mathsf{suc}(n)) \multimap \Diamond^{(p_1+p_2,\dots,p_k)}(n)\\
    \end{array}
  \end{mathpar}
\end{frame}
...
%  \youtem $(n, 0), v_n \models_{\TyNat} m$ if $n \geq m$.
